...
(function(e){function u(c,b,g){c=e("<div/>").html(c).text();var a=b.split(" ");b=b.substring(0,b.length-a[a.length-1].length);0==b.length&&(b="!padrenull");c=c.split("&");for(var a="",l=!1,n=!1,f=0;f<c.length;f++)"query="==c[f].substring(0,6)?(a+="query="+b,l=!0):null!=g&&"profile="==c[f].substring(0,8)?(a+="profile="+g,n=!0):a+=c[f],a+="&";l||(a=a+("query="+b)+"&");n||null==g||(a+="profile="+g,a+="&");return a.substring(0,a.length-1)}e.fn.fbcompletion=function(c){var b={collection:"funnelback_documentation",
show:10,sort:0,delay:0,length:3,alpha:.5,program:"/s/redirect",interactionLog:"/s/log",format:"simple",enabled:"disabled",standardCompletionEnabled:!0,logging:"enabled",dwellLogging:"enabled",dwellLoggingTimeout:3E3,selectLogging:"enabled",tmplId:"fb-completion-tmpl",profile:"_default",searchBasedCompletionEnabled:!1,searchBasedCompletionProgram:"/s/redirect",searchBasedCompletionProfile:null,searchBasedCompletionFunction:function(b,l,e){b=[];var f=l.term.split(" "),f=f[f.length-1],d=l.term.substring(0,
l.term.length-f.length);"|"==f.substring(0,1)&&(f=f.substring(f.indexOf(":")+1,f.length));var p=f.toLowerCase();e=e.response.facets;for(var r=1,q=0;q<e.length;q++){var m=e[q],c=m.name;if("undefined"!=typeof m.categories)for(var h=m.name.substring(0,f.length).toLowerCase()==p,g=0;g<m.categories[0].values.length;g++){var k=m.categories[0].values[g],t="|"+k.constraint+":"+k.label;k.label.substring(0,f.length)!=f&&!h||-1!=l.term.indexOf(t)||(k={action_t:"E",action:d+t+" ",value:d+k.label,category:c,rank:r,
matchOn:l.term,disp_t:"H",disp:k.label,wt:500},b.push({label:k.disp,value:k.value,extra:k,matchOn:k.matchOn,category:k.category,rank:k.rank}),r++)}}return b},zindex:1E3},g=function(a,e,c,f,d){if(a&&"enabled"===b.logging){a=b.interactionLog+"?collection="+b.collection+"&type="+encodeURIComponent(c)+"&client_time="+(new Date).getTime()+(""!==b.profile?"&profile="+b.profile:"");for(var p in f)a+="&"+encodeURIComponent(p)+"="+encodeURIComponent(f[p]);e?(d(),jQuery.ajax({type:"GET",url:a,error:function(b,
d,a){window.console&&console.log("Interaction log error: "+d+", "+a)}})):jQuery.ajax({type:"GET",url:a,error:function(b,a,f){window.console&&console.log("Interaction log error: "+a+", "+f);d()},success:function(b){d()}})}else d()};c&&e.extend(b,c);if("enabled"==b.enabled)return this.each(function(){var a=this,c,n;jQuery().template&&(n=0<jQuery("#"+b.tmplId).length?jQuery("#"+b.tmplId).template():jQuery("<script>[Error: Template <tt>"+b.tmplId+"</tt> not found]\x3c/script>").template());e(a).autocomplete({appendTo:0<
e("#fb-queryform").length?"#fb-queryform":"body",source:function(a,d){var e=[!b.searchBasedCompletionEnabled,!b.standardCompletionEnabled],c=[[],[]];b.searchBasedCompletionEnabled&&jQuery.ajax({type:"GET",url:b.searchBasedCompletionProgram+"?"+u(b.query,a.term,b.searchBasedCompletionProfile),dataType:"jsonp",error:function(b,a,d){window.console&&console.log("Autocomplete error: "+a+", "+d)},success:function(g){c[0]=b.searchBasedCompletionFunction(b,a,g);e[0]=!0;e[1]&&d(c[1].concat(c[0]))}});b.standardCompletionEnabled&&
jQuery.ajax({type:"GET",url:b.program+"?collection="+b.collection+"&partial_query="+a.term.replace(/ /g,"+")+"&show="+b.show+"&sort="+b.sort+"&alpha="+b.alpha+"&fmt="+("simple"==b.format?"json":"json++")+(""!==b.profile?"&profile="+b.profile:""),dataType:"jsonp",error:function(b,a,d){window.console&&console.log("Autocomplete error: "+a+", "+d)},success:function(b){for(var m=c[1],g=0;g<b.length;g++){var h=b[g];null!=h&&("string"==typeof h?m.push({label:h,matchOn:a.term,rank:g+1}):"object"==typeof h&&
m.push({label:h.disp?h.disp:h.key,value:"Q"==h.action_t?h.action:h.key,extra:h,matchOn:a.term,category:h.cat,rank:g+1}))}e[1]=!0;e[0]&&d(c[1].concat(c[0]))}})},open:function(){jQuery(this).autocomplete("widget").css("z-index",b.zindex);"undefined"!==typeof c&&clearTimeout(c);"enabled"===b.dwellLogging&&(c=setTimeout(function(f){f=e(a).val();g("enabled"===b.dwellLogging,!0,"dwell",{partial_query:f},function(){})},b.dwellLoggingTimeout));return!1},close:function(){"undefined"!==typeof c&&clearTimeout(c)},
delay:b.delay,minLength:b.length,select:function(f,d){var c=!1;!d.item.extra||"C"!=d.item.extra.action_t&&"E"!=d.item.extra.action_t||(c=!0);g("enabled"===b.selectLogging,c,"select",{action_type:d.item.extra.action_t,action:d.item.extra.action,value:d.item.value,category:d.item.category,rank:d.item.rank,partial_query:d.item.matchOn},function(){if(d.item.extra)switch(d.item.extra.action_t){case "C":eval(d.item.extra.action);break;case "U":document.location=d.item.extra.action;break;case void 0:case "":e(a).val(d.item.value);
e(a).context.form.submit();break;case "E":e(a).val(d.item.extra.action);var b=e(a);setTimeout(function(){b.autocomplete("search")},1);break;default:e(a).val(d.item.extra.action),e(a).context.form.submit()}else e(a).val(d.item.value),e(a).context.form.submit()});return!1}}).data("ui-autocomplete")._renderItem=function(b,a){var c;if(a.extra)switch(a.extra.disp_t){case "J":c=n?jQuery.tmpl(n,a.extra.disp).appendTo("<p></p>").parent().html():"[Error: jQuery template plugin is unavailable]";break;case "C":c=
eval(a.extra.disp);break;case "T":c=a.label.replace(new RegExp("("+a.matchOn+")","i"),"<strong>$1</strong>");break;default:c=a.label}else c=a.label.replace(new RegExp("("+a.matchOn+")","i"),"<strong>$1</strong>");return jQuery("<li></li>").data("item.autocomplete",a).append("<a>"+c+"</a>").appendTo(b)};e(a).data("ui-autocomplete")._renderMenu=function(a,b){var c=this,e="";jQuery.each(b,function(b,d){d.category&&d.category!=e&&(a.append('<li class="ui-autocomplete-category">'+d.category+"</li>"),e=
d.category);c._renderItemData(a,d)})}}),this}})(jQuery);

$(document).ready(function(){
	$("#search").fbcompletion({
		  'enabled'    : 'enabled',
      'standardCompletionEnabled': true,
      'collection' : 'york-uni-web',
      'program'    : '//york.funnelback.co.uk/s/suggest.json',
      'format'     : 'extended',
      'alpha'      : '.5',
      'show'       : '10',
      'sort'       : '0',
      ...
      'delay'      : '0',
      'profile'    : '',
      'query'      : 'query=&amp;collection=york-uni-web&amp;site=yorkweb',
